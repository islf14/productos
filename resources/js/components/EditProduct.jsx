...
const EditProduct = () => {
  ...
  const update = async (e) => {
    e.preventDefault();
    await Api.updateProduct(id, {description, price, stock}).then(
      response =>  {
  ...
  useEffect( () => {
    const getProductById = async () => {
      await Api.showProduct(id).then(
        response =>  {
          console.log(response);
          response.status == 200 ? console.log("Encontrado correctamente") : "";
  ...
  return (
    <div className=''>
      <h3 className='max-w-md mx-auto mb-4 text-lg'>Edit product</h3>
      <form onSubmit={update} className='max-w-md mx-auto'>
